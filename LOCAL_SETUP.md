...
1. Start MySQL (e.g. from XAMPP Control Panel or your stack).
2. Create a database and user (or use root with no password for local dev):
...
1. Put the project inside the web root (e.g. `htdocs/dcsmanila` or `www/dcsmanila`).
2. Ensure the database (and optional `.env`) are set as above.
3. Open: `http://localhost/dcsmanila` or `http://localhost/dcsmanila/login.php`.
